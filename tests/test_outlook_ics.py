...
def test_exportar_ics_contiene_los_campos_principales():
    _tarea_completa()
    ics = outlook_ics.exportar_ics(db.listar_tareas_outlook())
...
def test_roundtrip_ics_exportar_e_importar_de_vuelta():
    tid = _tarea_completa()
    ics = outlook_ics.exportar_ics(db.listar_tareas_outlook())
    ...
    resumen = outlook_ics.importar_ics(ics)
...
def test_importar_ics_reconoce_uid_y_actualiza_en_vez_de_duplicar():
    tid = db.crear_tarea_outlook("Tarea original", outlook_entry_id="UID-1")
    ics = (
        "BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//Test//ES\n"
    ...
    tareas = db.listar_tareas_outlook(estado="completada")
    assert len(tareas) == 1
    assert tareas[0]["id"] == tid
    assert tareas[0]["asunto"] == "Tarea renombrada"


def test_importar_ics_omite_vtodo_sin_asunto():
    ics = "BEGIN:VCALENDAR\nVERSION:2.0\nBEGIN:VTODO\nUID:X\nEND:VTODO\nEND:VCALENDAR\n"
    resumen = outlook_ics.importar_ics(ics)
    assert resumen == {"creadas": 0, "actualizadas": 0, "omitidas": 1}


def test_importar_ics_archivo_invalido_lanza_error_legible():
    with pytest.raises(outlook_ics.ErrorSincronizacionOutlook):
        outlook_ics.importar_ics("esto no es un ics")


# --- CSV ---------------------------------------------------------------------

def test_roundtrip_csv_exportar_e_importar_de_vuelta():
    _tarea_completa()
    csv_texto = outlook_ics.exportar_csv_outlook(db.listar_tareas_outlook())
    assert "Subject" in csv_texto.splitlines()[0]

    db.eliminar_tarea_outlook_definitivamente(db.listar_tareas_outlook()[0]["id"])
    resumen = outlook_ics.importar_csv_outlook(csv_texto)
    assert resumen == {"creadas": 1, "actualizadas": 0, "omitidas": 0}
...
def test_importar_csv_acepta_variaciones_de_fecha_y_encabezado():
    csv_texto = (
    ...
    t = db.listar_tareas_outlook()[0]
    assert t["estado"] == "en_progreso"
    assert t["prioridad"] == "alta"
    assert t["fecha_vencimiento"] == "2026-07-15"


def test_importar_csv_sin_columna_subject_lanza_error():
    with pytest.raises(outlook_ics.ErrorSincronizacionOutlook):
        outlook_ics.importar_csv_outlook("Foo,Bar\n1,2\n")


def test_importar_csv_omite_filas_sin_asunto():
